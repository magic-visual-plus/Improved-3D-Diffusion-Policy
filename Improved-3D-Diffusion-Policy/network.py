...
IMAGE_TYPE_RGB = 'RGB'
IMAGE_TYPE_DEPTH = 'Depth'
IMAGE_TYPE_INTRINSICS = 'Intrinsics'
...
class ZMQResponseClient(object):
    def __init__(self, host, port):
        self._host, self._port = host, port
        self._init_requester()

    def _init_requester(self):
    ...
    def send_request(self, request):
        ...
        self.socket.send(pickle.dumps(request, protocol=-1))
        response = self.socket.recv()
        return pickle.loads(response)
    

    def stop(self):
        logger.info('Closing the requester socket in {}:{}.'.format(self._host, self._port))
        self.socket.close()
        self.context.term()

class ZMQResponseServer(object):
    def __init__(self, host, port):
        self._host, self._port = host, port
    ...
    def send_response(self, response):
        ...
        self.socket.send(pickle.dumps(response, protocol=-1))

    def stop(self):
        logger.info('Closing the responder socket in {}:{}.'.format(self._host, self._port))
        self.socket.close()
        self.context.term()
...
class ZMQKeypointPublisher(object):
    ...
    def pub_keypoints(self, keypoint_array, topic_name):
        ...
        """
        logger.info('Publishing keypoints {} topic {}', keypoint_array, topic_name)
        buffer = pickle.dumps(keypoint_array, protocol = -1)
        self.socket.send(bytes('{} '.format(topic_name), 'utf-8') + buffer)

    def stop(self):
        logger.info('Closing the publisher socket in {}:{}.'.format(self._host, self._port))
        self.socket.close()
        self.context.term()
...
class ZMQCameraPublisher(object):
    ...
    def _init_publisher(self):
        self.context = zmq.Context()
        self.socket = self.context.socket(zmq.PUB)
        logger.info('tcp://{}:{}'.format(self._host, self._port))
        self.socket.bind('tcp://{}:{}'.format(self._host, self._port))
...
class ZMQCompressedImageTransmitter(object):
    ...
    def stop(self):
        logger.info('Closing the publisher in {}:{}.'.format(self._host, self._port))
        self.socket.close()
        self.context.term()
