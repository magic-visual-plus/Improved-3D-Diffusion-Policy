...
def servo_infer(policy: BasePolicy):
    ...
    while True:
        ...
        with torch.no_grad():
            ...
            if use_wrist:
                obs_dict_input['wrist_image'] = obs_dict['wrist_image'].permute(0, 3, 1, 2).unsqueeze(0)
            obs_dict_input['agent_pos'] = obs_dict['state'].unsqueeze(0)
            print(f'obs_dict_input{obs_dict_input}')
            print(f"agent_pos shape {obs_dict_input['agent_pos'].shape}, image shape {obs_dict_input['image'].shape}")
            action_list = []
            try:
...
def main(cfg):
    ...
    with Dbg_Timer("init policy"):
        ...
        logger.info("workspace type {}", type(workspace))
